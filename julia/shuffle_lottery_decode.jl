...
We get our data from the database. First, the behavior and then the cells associated with the behavior.
...
# ╔═╡ 169f0b33-e546-45e6-bd4f-6bbb204c04e4
md"""
## Visualize the results

To make visualization easier we gather the results in `bigdf` and also compute the Spearman rank correlation.
"""

# ╔═╡ 76f6f23b-16ff-4e7a-9c95-51e882b1f5cb
md"# Code Appendix"
...
"""
function lottery_sound_onset(raw_peh_list)
	pe_list = [peh(x)["vals"] for x in raw_peh_list]

	session_start_time = pe_list[1]["StartTime"]
	event_time(x) = begin
		ls = x["States"]["play_lottery_sound"][1]
...
"""
`count_spikes(ref, ts; start=0, stop=0.5)`

count spikes using searchsorted.

Note: this isn't exactly how the matlab code works. i think if there is an exact match then we might be off by one but i'm not totally sure. I think this is probably ok for us.  It's possible the matlab code was overkill?
"""
function count_spikes(ref::Vector, ts; start=0, stop=0.5)
	count_spikes.(ref, Ref(ts), start, stop)
end

# ╔═╡ f037ece0-bd3e-4590-9203-7ca4d1ca9322
function count_spikes(ref::Number, ts, start, stop)
	isfinite(ref) ? searchsortedlast(ts, ref+stop) - searchsortedlast(ts, ref+start) : NaN
end

# ╔═╡ 671649f7-c592-44fe-af96-4955fb73c683
function count_spikes(ref::Nothing, ts, start, stop)
	NaN
end

# ╔═╡ d9ed64d2-8042-41e4-bec2-1fdb62f7d60c
function count_spikes(ref::Missing, ts, start, stop)
	NaN
end
...
function one_perm(; trials_per_type=20, n_cells=20)
	...
	lm = fit(LinearModel, hcat(ones(6*trials_per_type), rTrain'), shuffle(label))
...
CSV.write("/Users/jerlich/Downloads/fof_shuff_pop.csv", bigdf)

# ╔═╡ 668dbf49-e4f4-4188-9165-703b7b84e8ab
JLD2.save("/Users/jerlich/Downloads/fof_shuff_pop.jld2",bigD)

# ╔═╡ a9a5c7ab-5fdb-4521-b1ea-df8f3226f334
bigD["32"]

# ╔═╡ 63f85be7-bd82-4331-b294-c7b7fe5812da
JSON3.write("/Users/jerlich/Downloads/fof_shuff_pop.json",bigD)
...
git-tree-sha1 = "2f962011721ba26f3b3467b05610a08cfd2caff5"
