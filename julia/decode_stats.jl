...
# ╔═╡ beb45ee2-f4fd-11ed-3575-afcecc57afed
using CSV, HypothesisTests, DataFrames, StatsBase, AlgebraOfGraphics, CairoMakie, Distributions

# ╔═╡ fe8f2184-ada0-4511-be5a-3bfb309f9052
begin
	fof = CSV.read("/Users/jerlich/Downloads/fof_pop.csv", DataFrame)
	shuff = CSV.read("/Users/jerlich/Downloads/fof_shuff_pop.csv", DataFrame)
	fof[:,"label"].="data"
	shuff[:,"label"].="shuffle"
	df = vcat(fof, shuff)
...
# ╔═╡ f16b1f2d-179f-446a-8b5e-08ca26a21ae4
md"""
This is wrong: we need to just compare the overlap of the two distributions. Classic error of bootstrapping something and then using a ttest... it will alway come out positive!
"""

# ╔═╡ 65a6d336-169d-4fb7-b112-fb0f06a49400
testdict = let 
	N = unique(fof.n)
	D = Dict{Int,HypothesisTests.HypothesisTest}()
	for n ∈ N
		this_decode = fof.Pearson[fof.n.==n]
		this_shuff = shuff.Pearson[fof.n.==n]
		t = MannWhitneyUTest(this_decode, this_shuff) #, mean, 20000)
		D[n] = t
	end
	D
end

# ╔═╡ e2ff1547-3a16-4f29-8a94-ec437679877c
testdict2 = let 
	N = unique(fof.n)
	D = Dict{Int,Any}()
	α = 0.05
	f(x) =  quantile(x, [α/2, 1 - α/2])
	for n ∈ N
		this_decode = f(fof.MSE[fof.n .== n])
		this_shuff = f(shuff.MSE[shuff.n .== n])
		D[n] = (this_decode, this_shuff)
	end
	D
end

# ╔═╡ d2c6895f-457e-4bd7-bab9-2b0a78a9110b
for (k,v) in testdict
	println("$k: $(pvalue(v))")
end


# ╔═╡ b0292f3e-d577-4ec3-a9d1-12a98189ada7
p = let
	f = Figure(resolution=(400,400))
	ax = Axis(f[1,1]; xlabel="Population size", ylabel="MSE (x 10⁴)")
	p=data(df) * mapping(:n=>log2=>"log₂ Neurons", :MSE=>x->x/1e4=>"MSE (x 10⁴)"; color=:label, dodge=:label) * visual(BoxPlot; show_notch=true)
	draw!(ax, p)
	f
end

# ╔═╡ ca2df35c-cac5-493e-acf4-465d209d0dd1
p2 = with_theme(resolution=(400,250)) do
	p = data(df) * mapping(:n=>log2=>"log₂ Neurons", :MSE=>(x->(x/120))=>"MSE"; color=:label, dodge=:label) * visual(BoxPlot; show_notch=true)
	draw(p)
end

# ╔═╡ b47d7410-eded-4be4-ae6f-28e40285acb1
save("/Users/jerlich/Downloads/MSE.svg", p2)

# ╔═╡ 3ba0b49a-6be8-45fa-b2ad-34a8d8763405
data(df) * mapping(:n=>log2=>"log₂ Neurons", :MSE=>(x->x/1e4)=>"MSE (x 10⁴)"; color=:label, dodge=:label) * visual(BoxPlot; show_notch=true) |> draw

# ╔═╡ da0eaf3f-c362-4eca-acce-94ea49c43ce5
draw(p)

# ╔═╡ c4fcb995-33d9-427c-b29a-f6560ae63352
foo = let
	n = []
	p = []
	D1 = Normal(0,1)
	...
	for s = 10:100:10000
		...
		s2 = rand(D2, s)
		push!(n,s)
		t = ApproximatePermutationTest(s1,s2, f, 2000)
		push!(p, pvalue(t))
	end
	DataFrame(;n,p)
end

# ╔═╡ 44b36b6e-2da4-4c96-a809-8f9940d2e816
let
	p = data(foo) * mapping(:n, :p=>log10) * visual(Lines)
	a = (;title="ApproximatePermutationTest")
	draw(p, axis=a)
end

# ╔═╡ 92771f4d-55c0-40c9-8395-713ba016be0e
4
...
manifest_format = "2.0"
project_hash = "208096382972a7174bf03ceacb2c057a10e42546"
