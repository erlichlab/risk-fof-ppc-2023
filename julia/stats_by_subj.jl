...
# ╔═╡ 7cd75df0-f753-11ed-040b-0b748245f3ca
using CSV, DataFramesMeta, MixedModels, GLM, PlutoUI, Dates

# ╔═╡ 9c72c734-2f34-4d26-8677-6d53fd80924e
using AlgebraOfGraphics, CairoMakie, StatsBase

# ╔═╡ e8fce794-932c-4621-95dc-29378295831b


# ╔═╡ d5f52d33-d3c2-4c7c-a1a8-44bc8d388550
md"""
## Code Appendix
"""

# ╔═╡ c30e38bf-dcdc-40ca-a1a4-2544d2d5ed41
PlutoUI.TableOfContents(include_definitions=true)

# ╔═╡ bbffa221-45b7-49a1-b4dc-82a2e82f5478
control_data() = begin
 df = CSV.read("../csv/figure_behavior_population.csv", DataFrame)
end


# ╔═╡ 2f4cd135-f72a-4fcc-8232-aed05f65aebc
"""
`combine_data(region, n)`
combines infusion and control data. region is `fof/ppc`, n is `uni/bi`.
"""
combine_data(region, n) = begin
	 df = vcat(
		 	control_data(), 
		 	@subset(
		 		CSV.read("../csv/figure_infusion_$(n)_$(region).csv", DataFrame), 
		 		:trialtime .< "2020-11-19",:dosage.>0,:dosage .<0.5)
	 		)
	 if region == "fof" && n == "bi"
		 @subset!(df, (:dosage .< 0.1) .| (:dosage .> 0.2))
	 end
	 @subset(df, :forced.==0, :RT .<3)
end

# ╔═╡ ae6a3080-1d39-4441-ab24-6d7ef15dd707
df_dict = let 
	Dict(
	:uni_ppc=>combine_data("ppc","uni"),
	:bi_ppc=>combine_data("ppc","bi"),
	:bi_fof=>combine_data("fof","bi"),
	:uni_fof=>combine_data("fof","uni")	
)
end;

# ╔═╡ 34fb8b08-7ddd-4b8e-be10-729b61e24150
unique(combine_data("fof","bi").dosage)

# ╔═╡ bb9ee35d-8408-4dd8-a80a-ed6678f0038c
opto_data(n) = begin
	f(x) = x == "NA" ? NaN : parse(Float64, x)
	@chain CSV.read("../csv/opto_$(n)lateral.csv", DataFrame) begin
		@transform(:forced = (:lottery_poke .== "NA") .| (:surebet_poke .== "NA"))
		@rtransform(:RT = f(:RT_choice))
	end
	
end

# ╔═╡ 43864af0-03dc-4eac-8389-d50b8015e8c0
opto_dict=Dict(
	:uni_fof_opto=>opto_data("uni"),
	:bi_fof_opto=>opto_data("bi"),
);

# ╔═╡ 80a98ebe-1b75-4a09-8421-ba7f3a70db36
begin
	f = @formula(choice ~ delta_ev * dosage)
	fr = @formula(choice ~ delta_ev)
end

# ╔═╡ 433a551a-627d-4fd0-869a-e154fa6a7c9b
"""

"""
function choice_effect(keyname::Symbol, d::Dict, subjid)
	try
	df = d[keyname]
	tdf = df[df.subjid .== subjid,:]
	mf = glm(f, tdf, Bernoulli())
	mr = glm(fr, tdf, Bernoulli())
	lrt = lrtest(mf, mr)
	v = coef(mf)[3:4]
	p = DataFrame(coeftable(mf))."Pr(>|z|)"[3:4]
	return (; subjid, exp=keyname, dosage=v[1], dosage_p=p[1],
			  ΔEVxdose=v[2], ΔEVxdose_p=p[2], lrt_p= lrt.pval[2], 
			  n_total=nrow(tdf), n_inf=sum(tdf.dosage.>0))
	catch
		@error keyname, subjid
		return (;subjid, exp=keyname, dosage=NaN, dosage_p=1, 
	  ΔEVxdose=NaN, ΔEVxdose_p=1, lrt_p= 1, n_total=0, n_inf=0)
	end
	
end

# ╔═╡ a5e01516-b834-468f-a371-a225c65af926
"""
`stats(d, f=choice_effect)`
d is a dictionary of dataframes
f is the function to call. it should take 3 inputs (symbol, dictionary, subjid)
"""
function stats(d::Dict, f=choice_effect)
	
	z = [
		mapreduce((y)->f(x,d,y), vcat,unique(d[x].subjid))
		for x in keys(d)
	]
	@chain DataFrame(vcat(z...)) begin
		sort([:exp,:subjid])
	end
end


# ╔═╡ ac466bf6-2e1a-4ff3-b7a6-4c2b843fabe4
musc_stats_df = select(stats(df_dict), :exp,:subjid,:lrt_p, :)

# ╔═╡ 22f5d3cc-92dd-43f7-b87e-ab185f706645
@subset(musc_stats_df, :subjid.==2152)

# ╔═╡ a5596028-9d45-40b5-b427-4cd957063a53
CSV.write("/Users/jerlich/Downloads/musc_stats_jl.csv" , musc_stats_df)

# ╔═╡ 483febd8-8d53-455a-9ccf-6b42dea30b41
subj_plots(k, d) = begin

	df = @by(d[k], [:subjid, :delta_ev, :dosage], :μ=mean(:choice))
	df[:,"subj"] .= string.(df.subjid)
	df[:,"dose"] .= string.(df.dosage)
	
p = data(df) * mapping(:delta_ev=>"ΔEV", :μ=>"P(Lottery)", color=:dose, layout=:subj) * (visual(Scatter))
end

# ╔═╡ 3ba1859b-5a8f-4b37-99d5-8e86e071c5a1
draw(subj_plots(:uni_ppc, df_dict))

# ╔═╡ b5ccb5b8-ec69-43e3-bc8b-c4ae6dd77ad7
draw(subj_plots(:uni_fof, df_dict))

# ╔═╡ c464d628-315b-4ace-8b93-5df0da9710fe
draw(subj_plots(:bi_fof, df_dict))

# ╔═╡ 9d605aa0-37f7-4bed-97c8-32190c8f9b91
draw(subj_plots(:bi_ppc, df_dict))
...
manifest_format = "2.0"
project_hash = "bdfd2f6e7c08d1d7e201c0a513ae9837a36fd09c"
...
git-tree-sha1 = "f83f0c73074e07e5f2fecaebd1afaae19a78460c"
